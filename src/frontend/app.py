...
Point it at a different backend with:
    RAG_API_URL=http://your-host:8000 streamlit run src/frontend/app.py

NOTE ON TESTING: streamlit isn't installable in this sandbox (confirmed,
same as fastapi/qdrant-client). This file is syntax-checked only. The
HTTP client it depends on (api_client.py) IS tested against a mock
server -- that's the part actually likely to have bugs (wrong field
names, wrong status-code handling); the untested part here is Streamlit
rendering calls, which either work as documented or don't.
"""
